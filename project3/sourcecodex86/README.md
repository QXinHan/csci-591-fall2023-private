# Project3

- **Name :** Zhuoyun Qian
- **Email :** 3290862073@qq.com
- **Due :** 8/12/2023

**Link to assignment : **[https://fangtian-zhong.github.io/project3.html](https://fangtian-zhong.github.io/project3.html)

## Task 1 : get `program1.exe` 

_The `program1.exe` is a simple executable file to print `"Hello program1"`_

## Task 2 : get `program3.exe`

- **Summary**

_The function of the `program3.exe` is to add a new section at the end of `program2.exe` , the section contains the encrypted data of `program1.exe`_

* **Solution**

_I write `program3.cpp` , the steps as follow：_

1. Read the data of `program1.exe` in the buffer.
2. Encrypt it by XOR `0x40`.
3. Parse `program2.exe` to get the section  information.
4. The `NumberOfSections` plus one , `SizeOfImage` plus the `size of program1.exe`.(`PIMAGE_NT_HEADERS`)
5. Locate the new section (calculate the `Rva` of it).(`PIMAGE_SECTION_HEADER`)
6. Write attributes of new section in the structure `IMAGE_SECTION_HEADER`.
7. Copy the buffer to the end of `program2.exe`.(`memcpy`)

## Task 3 : get `program.exe`

* **Summary**

_This is main part of the project._

* **Solution**

1. I develop `program1.cpp`,the function of it is to print `"hello program1"`.
2. I develop `program3.cpp`,the function of it is to add a section `.shell` at the last of program2.exe.The data of section `.shell` is the encrypted `program1.exe`.
3. I develop `program2.cpp`,the function as follows:
   * print `"hello program2"`,I think it is a symbol,giving the user a signal that the `program2.exe` runs correctly.
   * read and decrypt the last section of `program2.exe` to get `program1.exe`.`GetModuleHandle`
   * parse the `program1.exe` for its OEP and ImageBase.
   * stretch the `program1.exe` and load it in the buffer.
   * use API `CreateProcess` to create a suspended process.
   * get the context of suspended process,aiming to get the OEP and ImageBase stored in register.
   * uinistall the image of `program2.exe` in the suspended process.`NtUnmapViewOfSection`
   * use API `VirtualAllocEx`to allocate memory at the specified location which is "ImageBase" of `program1.exe` for `program1.exe` in the suspended process . Maybe need to repair the relocation table of the `program1.exe`.
   * use API `WriteProcessMemory` to copy the `program1.exe` to the suspended process.
   * change the context of the `program2.exe`( the image of it is uninstalled ).Change the base to "Imagebase" of `program1.exe` and change OEP to the "OEP" of program1.exe.
   * set the context and return to the main thread.`SetThreadContext` and `ResumeThread`
   * SUCCESS! 

* **Conclusion**

_It is difficult if you learned little about OS like me , it is very hard to finish this project . But you would learn a lot about OS after finishing it._

